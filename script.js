    ...
    /* VENTANITA DE CADA MONITO — SE ABRE AL HACER CLIC, NO NAVEGA DIRECTO */
    const figurasItem = document.querySelectorAll('.figura-item');

    function cerrarFiguras(excepto) {
      figurasItem.forEach(item => {
        if (item === excepto) return;
        item.classList.remove('activa');
        item.querySelector('.figura-escenario').setAttribute('aria-expanded', 'false');
      });
    }

    figurasItem.forEach(item => {
      const boton = item.querySelector('.figura-escenario');

      boton.addEventListener('click', () => {
        const yaActiva = item.classList.contains('activa');
        cerrarFiguras(item);
        item.classList.toggle('activa', !yaActiva);
        boton.setAttribute('aria-expanded', String(!yaActiva));
      });
    });

    document.addEventListener('click', (e) => {
      if (!e.target.closest('.figura-item')) cerrarFiguras(null);
    });

    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') cerrarFiguras(null);
    });
